...
Project Overview This project covers:
...
Install ModSecurity sudo apt update sudo apt install libapache2-mod-security2 -y

Copy default ModSecurity config sudo cp /etc/modsecurity/modsecurity.conf-recommended /etc/modsecurity/modsecurity.conf

Enable Blocking Mode sudo sed -i 's/SecRuleEngine DetectionOnly/SecRuleEngine On/' /etc/modsecurity/modsecurity.conf

Enable CRS rules in security2.conf /etc/apache2/mods-enabled/security2.conf

Restart Apache sudo systemctl restart apache2

Custom Rule SecRule ARGS "@rx (?i)union\s+select"
"id:10001,phase:2,block,msg:'SQL Injection Attempt: UNION SELECT detected'"

Testing Attacks from Kali (Attacker Machine) SQL Injection Test Input: test' UNION SELECT 1,2,3--

Check logs on Ubuntu: sudo tail -f /var/log/apache2/error.log
